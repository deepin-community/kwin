...
/*KWin::Effect * getEffect(const QString &name)
{
    if (!effects)
        return nullptr;
...
QObjectList KWinUtils::clientList()
{
    ...
    // for (KWin::X11Client* c : clients) {
    //     list << static_cast<QObject*>(c);
    // }
...
{
    // KWin::Workspace *ws = Workspace()::sef;
    // if (ws) {
...
{
    // QObject *multitasking = KWinUtils::getEffect("multitaskview");
...
{
    // QObject *presentWindows = KWinUtils::getEffect("presentwindows");
