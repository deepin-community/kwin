    ...
    , m_timer(new QTimer(this))
{
    ...
    a->setObjectName(QStringLiteral("ShowMultiView"));
        ...
        if (m_bgSlidingStatus) {
            ...
            if (lwkobj && cwkobj) {
                ...
                if (effects->waylandDisplay()) {
                    ...
                    if (screenId != effects->getCurrentPaintingScreen()) {
                        if (area.x() > 0)
                            continue;
                    }
                } else {
    ...
    case QEvent::MouseButtonRelease:
        ...
        if (!m_wasWindowMove && m_dockRect.contains(mouseEvent->pos())) {
            ...
            if (QX11Info::isPlatformX11()) {
                ...
                m_effectFlyingBack.begin();
                effects->addRepaintFull();
                effects->sendPointer(mouseEvent->button());
            }
...
void MultitaskViewEffect::grabbedKeyboardEvent(QKeyEvent* e)
{
    ...
    if (e->type() == QEvent::KeyPress) {
        ...
        case Qt::Key_AsciiTilde:
            ...
            break;
        default:
            if (!QX11Info::isPlatformX11() && effects->isShortcuts(e)) {
...
void MultitaskViewEffect::setActive(bool active)
{
    ...
    if (!QX11Info::isPlatformX11()) {
        effects->changeBlurState(active);
    }
...
void MultitaskViewEffect::removeDesktop(int desktop)
{
    ...
    for (const auto &ew : effects->stackingOrder()) {
        ...
        if (effects->isTransientWin(ew) && !ew->isModal()) {
            continue;
        }
        newd = dl[0] == 1 ? 1 : dl[0] - 1;
...
void MultitaskViewEffect::moveWindowChangeDesktop(EffectWindow *w, int todesktop, EffectScreen *toscreen, bool isSwitch)
{
    ...
    childWinList = effects->getChildWinList(w);
...
{
    if (effects->getChildWinList(w).size() == 0) {
        w->closeWindow();
        return true;
    }
    return false;
...
{
    if (w->keepAbove()) {
        effects->setKeepAbove(w, false);
    } else {
        effects->setKeepAbove(w, true);
    }
